...
if(isset($_POST['login'])) {
    ...
    $sql = "SELECT * FROM `registerd_users` WHERE email='$email' AND password='$pass'";
    $res = mysqli_query($conn, $sql);

    if(mysqli_num_rows($res) == 1) {
        $row = mysqli_fetch_assoc($res);
        $_SESSION['userid'] = $row['id']; // Setting userid in session
        header('Location: index.php'); // Redirecting after successful login
        exit();
    } else {
        // Set error message
...
<div class="login-page">
    ...
    <div class="welcoming">
    <h4>Welcome to online dairy!Please Login</h4>
    
    <p>New member?<span class="register-btn"><a href="register.php">Register</a></span> here</p>
    </div>
